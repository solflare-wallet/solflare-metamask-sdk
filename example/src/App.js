...
export default class App extends Component {
  ...
  handleSignTransaction = async () => {
    try {
      const tx = await this.solflare.signTransaction(
        await this.createTransaction(this.solflare.publicKey)
      );
  ...
  handleSignAllTransactions = async () => {
    try {
      const txs = await this.solflare.signAllTransactions([
        await this.createTransaction(this.solflare.publicKey),
        await this.createTransaction(this.solflare.publicKey)
      ]);
  ...
  handleSignMessage = async () => {
    try {
      const signature = await this.solflare.signMessage(
  ...
  handleSignAndSendTransaction = async () => {
    try {
      const tx = await this.solflare.signAndSendTransaction(
        await this.createTransaction(this.solflare.publicKey)
      );
